...
function detectAmenities(property) {
  // Prefer real amenities from the backend now that it sends them.
...
export function enrichProperty(property, index = 0) {
  ...
  return {
    ...
    rating,
    amenities,
    // Fall back to location_name when the API doesn't send a street address,
    // so downstream search/display code always has something usable.
    address: property.address || property.location_name || '',
    city: property.city === 'undefined' ? '' : (property.city || ''),
    deposit: price > 0 ? Math.round(price * (0.4 + (hash % 3) * 0.2)) : 0,
    metroDistance,
    collegeDistance,
    badges,
    // Use the real gender_filter from the backend when present; only fall
    // back to a random guess if it's genuinely missing.
    gender: property.gender_filter
      ? (property.gender_filter === 'Men' ? 'Male' : property.gender_filter === 'Women' ? 'Female' : 'Unisex')
      : genders[hash % 3],
    sharingType: sharingTypes[hash % 4],
    isVerified: hash % 3 !== 0,
    isCertified: hash % 7 === 0,
...
export function filterAndSortProperties(properties, filters) {
  ...
  let result = properties.filter((p) => {
    const search = (searchTerm || location || '').toLowerCase();
    // Guard every field with a fallback — API responses won't always include
    // every field (e.g. no `address`), so never call .toLowerCase() on
    // something that might be undefined.
    const matchesSearch =
      !search ||
      (p.name || '').toLowerCase().includes(search) ||
      (p.address || '').toLowerCase().includes(search) ||
